...
vector<string> splitString(string s1, string delim) {
    ...
    while (pos != std::string::npos) { // use find() function to get the position of the delimiters
...
enum enMainMenuOptions {
    eListClients = 1, eAddNewClient,
    eDeleteClient, eUpdateClient,
    eFindClient, eTransacionsMenu,
    eManageUsers, eLogout
};
struct stClient {
...
bool checkAccessPermissions(enMainMenuPermissions permission) {
    ...
    return currentUser.permissions == enMainMenuPermissions::eAll ? 1 : (result == permission ? 1 : 0);
}
...
stClient readNewClient() {
    ...
    while (clientExistsByAccountNumber(client.accountNumber))
        client.accountNumber = readString("\nClient with [" + client.accountNumber + "] already exists, enter another an account number: ");
    read(client);
...
void addNewClients() {
    ...
    do {
        ...
        addNewClient();
        cout << "\nThe client is added successfully, do you want to add more clients (y / n)?\n";
        cin >> addMore;
        cout << endl << endl;
...
void saveCleintsDataToFile(vector<stClient> vClients) {
    ...
    if (myFile.is_open()) {
        for (stClient c : vClients)
            if (!c.markForDelete) {
                dataLine = convertRecordToLine(c); //we only write records that are not marked for delete.
...
enum enTransacionsMenuOptions { eDeposite = 1, eWithdraw, eTotalBalances, eMainFromTrans };
...
void depositAccountToClientByAccountNumber(double depositedAmount, string AccountNumber, vector<stClient>& vClients) {
    char answer;
...
void showDepositClientScreen() {
    ...
    depositAccountToClientByAccountNumber(depositedAmount, accountNumber, vClients);
...
void showWithdrawClientScreen() {
    ...
    while (withdrawAmount > client.accountBalance)
        withdrawAmount = readNum("Amount exceeds the balance, you can't withdraw up to: " + to_string(client.accountBalance) + ".\n");
    depositAccountToClientByAccountNumber(withdrawAmount * (-1), accountNumber, vClients);
    goBackToTransactionsMenu();
}
...
void showTransactionsMenu() {
    ...
    cout << "\t[4] Main menu.\n";
    cout << "===========================================\n";
    perfromTransacionsOption((enTransacionsMenuOptions)short(readNum("Choose what you want to do [1 to 4]: ")));
}
enum enManageUsersMenuOptions {
    eListUsers = 1, eAddNewUser,
    eDeleteUser, eUpdateUser,
    eFindUser, eMainFromManage = 6
};
...
void changePermission(char answer, string question, short& permissions, enMainMenuPermissions ePermissions) {
    cout << question;
    cin >> answer;
    if (tolower(answer) == 'y')
...
int readPermissionsToSet() {
    ...
    changePermission(answer, "\nShow client list (y/n)? ", permissions, enMainMenuPermissions::pListClients);
    changePermission(answer, "Add new client (y/n)? ", permissions, enMainMenuPermissions::pAddNewClient);
    changePermission(answer, "Delete client (y/n)? ", permissions, enMainMenuPermissions::pDeleteClient);
    changePermission(answer, "Update client (y/n)? ", permissions, enMainMenuPermissions::pUpdateClient);
    changePermission(answer, "Find client (y/n)? ", permissions, enMainMenuPermissions::pFindClient);
    changePermission(answer, "Transactions (y/n)? ", permissions, enMainMenuPermissions::pTransactions);
    changePermission(answer, "Manage users (y/n)? ", permissions, enMainMenuPermissions::pManageUsers);
...
void addNewUsers() {
    ...
    do {
        ...
        addNewUser(vUsers);
        cout << "\nThe user is added successfully, do you want to add more users (y / n)?\n";
        cin >> addMore;
        cout << endl << endl;
    } while (toupper(addMore) == 'Y');
}
...
void saveUsersDataToFile(vector<stUser> vUsers) {
    ...
    if (myFile.is_open()) {
        for (stUser u : vUsers)
            if (!u.markForDelete) {
                dataLine = convertRecordToLine(u); //we only write records that are not marked for delete.
                myFile << dataLine << endl;
            }
        myFile.close();
    }
...
void showManageUsersMenu() {
    ...
    cout << "\t[6] Main menu.\n";
    cout << "===========================================\n";
    performManageUsersMenuOption((enManageUsersMenuOptions)short(readNum("Choose what you want to do [1 to 6]: ")));
}
...
void showMainMenu() {
    ...
    cout << "\t[8] Logout.\n";
    cout << "===========================================\n";
    performMainMenuOption((enMainMenuOptions)short(readNum("Choose what you want to do [1 to 8]: ")));
}
void showLoginScreen() {
    string userName, password;
    bool loginFailed = 0;
